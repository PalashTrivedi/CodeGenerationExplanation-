...
with TAB1:
    ...
    USER_INPUT = st.text_input(
        "Enter text prompt below", max_chars=100,
        placeholder="Write a code to print hello world.", key="123")
    if st.button("Generate"):
        with st.spinner("Please wait"):
            if len(USER_INPUT) <= 10:
                ...
                RESPONSE = generate(
                    PROMPT, max_tokens=300, temperature=0.7).replace("--", "")
                st_ace(value=RESPONSE,language  = "python",
                       height=320, theme="chrome", readonly=True)


with TAB2:
    st.markdown("Enter code below...")
    TEXT = st_ace(language='python', theme='xcode',key = "b")
    if st.button("Explain"):
        with st.spinner("Please wait"):
            PROMPT = open_files(
                "./codeexplain.txt").replace("<<USER_INPUT>>",TEXT)
            RESPONSE = explain(
                PROMPT, max_tokens=300, temperature=0.7).replace("--", "")
            st.text_area(label="Explanation",
                         value=RESPONSE, height=320)